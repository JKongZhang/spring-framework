...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		// 没有缓存，则尝试从配置文件中获取 构造方法 和 构造方法参数
		if (constructorToUse == null || argsToUse == null) {
			...
			if (explicitArgs != null) {
				...
				// 从 BeanDefinition 中获取构造函数的参数，也就是从配置文件中提取构造参数
			...
			for (Constructor<?> candidate : candidates) {
				...
				if (resolvedValues != null) {
					try {
						// 根据构造函数注解获取参数名称 {@link ConstructorProperties} annotation.
	 ...
	 * <p>
	 * Implementation requires iterating over the static or instance methods with the name specified
	 * in the RootBeanDefinition (the method may be overloaded) and trying to match with the parameters.
	 * We don't have the types attached to constructor args, so trial and error is the only way to go here.
	 * The explicitArgs array may contain argument values passed in programmatically via the corresponding getBean method.
	 *
	 * @param beanName     the name of the bean
	 * @param mbd          the merged bean definition for the bean
	 * @param explicitArgs argument values passed in programmatically via the getBean method,
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		// 初始化 BeanWrapperImpl，向BeanWrapper对象中添加 ConversionService 对象和属性编辑器 PropertyEditor 对象
		this.beanFactory.initBeanWrapper(bw);
		...
		if (factoryBeanName != null) {
			...
			factoryBean = null;
			// TODO: 2020-03-24 此处为什么获取的是 factory class ？
			factoryClass = mbd.getBeanClass();
			// 设置静态工厂方式为true
			isStatic = true;
		}

		// 获得 factoryMethodToUse: 静态工厂中指定创建实例的方法
